...
def extract_experience(driver):
    """
    Extracts the 'Experience' section from the LinkedIn profile based on the updated DOM structure.
    """
    experience_data = []
    ...
    try:
        ...
        # Each experience/job is inside an <a> with data-field="experience_company_logo"
        experience_elements = driver.find_elements(
            By.XPATH, '//a[@data-field="experience_company_logo"]'
        )

        for exp in experience_elements:
            try:
                # Job title
                job_title = exp.find_element(
                    By.XPATH, './/div[contains(@class,"hoverable-link-text t-bold")]/span[@aria-hidden="true"]'
                ).text.strip()
            except:
                job_title = None

            try:
                # Company name and role (like "CASH PLUS · Stage")
                company_name = exp.find_element(
            ...
            try:
                # Dates (like "juin 2023 - août 2023 · 3 mois")
                dates = exp.find_element(
                    By.XPATH, './/span[contains(@class,"t-14 t-normal t-black--light")][1]/span[@aria-hidden="true"]'
                ).text.strip()
            except:
                dates = None

            try:
                # Location (like "Casablanca, ...")
                location = exp.find_element(
                    By.XPATH, './/span[contains(@class,"t-14 t-normal t-black--light")][2]/span[@aria-hidden="true"]'
                ).text.strip()
            except:
                location = None
        ...
        logging.error(f"Error extracting experience section: {e}")
